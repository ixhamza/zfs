 ...
 * do_unmount() passes its flags straight to the unmount syscall, a caller that
 * set UMOUNT_NOFOLLOW had that bit misread as MS_CRYPT (an unintended
 * encryption-key unload), and MS_CRYPT/MS_OVERLAY could likewise leak to the
 * kernel.  They now use high bits that alias nothing in the unmount flag set
 * and are masked off in do_unmount() before the syscall.
 *
