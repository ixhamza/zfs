    ...
    vdev_t *vd, int l, uint64_t txg, int flags)
{
	 ...
	 */
	if (vd->vdev_isspare || vd->vdev_isl2cache) {
	...
	for (int i = 0; i < 2; i++) {
		if (sav[i]->sav_label_sync) {
			for (int v = 0; v < sav[i]->sav_count; v++) {
				uint64_t *good_writes;
				good_writes = kmem_zalloc(sizeof (uint64_t),
				    KM_SLEEP);
				zio_t *vio = zio_null(zio, spa, NULL,
				    vdev_label_sync_ignore_done, good_writes,
				    flags);
				vdev_label_sync(vio, good_writes,
				    sav[i]->sav_vdevs[v], l, txg, flags);
				zio_nowait(vio);
			}
		}
	}
