...
vdev_label_init(vdev_t *vd, uint64_t crtxg, vdev_labeltype_t reason)
{
	...
	if (reason_spare || reason_l2cache) {
		...
		/*
		 * When aux added during pool creation, spa->spa_uberblock
		 * does not get populated until then. Thus, we have to sync
		 * it on next config sync
		 */
		if (uberblock_verify(&spa->spa_uberblock))
			vd->vdev_aux_uber_sync = B_TRUE;
	} else {
		uint64_t txg = 0ULL;
...
vdev_uberblock_sync_list(vdev_t **svd, int svdcount, uberblock_t *ub, int flags)
{
	...
	/*
	 * Check if we need to sync spare or cache vdev
	 */
	for (int v = 0; v < spa->spa_spares.sav_count; v++) {
		aux_vd = spa->spa_spares.sav_vdevs[v];
		if (aux_vd->vdev_aux_uber_sync) {
			vdev_uberblock_sync(zio, &good_writes, ub, aux_vd,
			    flags);
		}
	}
	for (int v = 0; v < spa->spa_l2cache.sav_count; v++) {
		aux_vd = spa->spa_l2cache.sav_vdevs[v];
		if (aux_vd->vdev_aux_uber_sync) {
			vdev_uberblock_sync(zio, &good_writes, ub, aux_vd,
			    flags);
		}
	...
	for (int v = 0; v < spa->spa_spares.sav_count; v++) {
		aux_vd = spa->spa_spares.sav_vdevs[v];
		if (aux_vd->vdev_aux_uber_sync && vdev_writeable(aux_vd)) {
			aux_vd->vdev_aux_uber_sync = B_FALSE;
			zio_flush(zio, aux_vd);
		}
	}
	for (int v = 0; v < spa->spa_l2cache.sav_count; v++) {
		aux_vd = spa->spa_l2cache.sav_vdevs[v];
