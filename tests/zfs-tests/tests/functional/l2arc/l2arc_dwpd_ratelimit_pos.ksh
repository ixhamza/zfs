...
#	3. Fill L2ARC with staged writes to complete first pass.
#	4. Measure writes over test period.
#	5. Repeat 1-4 for DWPD values 0, 1800, 5000, 10000.
#	6. Verify DWPD=0 > DWPD=10000 > DWPD=5000 > DWPD=1800.
#
...
for dwpd in 0 10000 5000 1800; do
	...
	log_must dd if=/dev/urandom of=/$TESTPOOL/file1 bs=1M count=$((fill_mb/3))
	log_must sleep 5
	log_must dd if=/dev/urandom of=/$TESTPOOL/file2 bs=1M count=$((fill_mb/3))
	log_must sleep 5
	log_must dd if=/dev/urandom of=/$TESTPOOL/file3 bs=1M count=$((fill_mb/3))
	log_must sleep 5
